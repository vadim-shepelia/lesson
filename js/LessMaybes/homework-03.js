...
function changeObj(obj) {
  ...
  obj.hobby = "skydiving";
  obj.premium = false;
  //не поняв яз завдяки Object.keys() вивести ключ:значение (пох як главне шо робило б ?)
...
const findBestEmployee = function (employees) {
  ...
  for (const key in employees) {
    if (employees[key] === arrValue.reduce((a, b) => Math.max(a, b), -Infinity))
      return key;
  }
};
...
const countTotalSalary = function (employees) {
  let arrSum = Object.values(employees);
  //console.log(typeof arrSum[1])
  let a = 0;
...
const calculateTotalPrice = function (allProdcuts, productName) {
  for (let i = 0; i < Object.values(allProdcuts).length; i++) {
    if (Object.values(allProdcuts)[i].name === productName) {
      let sum =
        Object.values(allProdcuts)[i].price *
        Object.values(allProdcuts)[i].quantity;
      return sum;
    }
  }
};
...
Вызовы функции для проверки работоспособности твоей реализации.
  */
// console.log(calculateTotalPrice(products_1, "Радар")); // 5200

// console.log(calculateTotalPrice(products_1, "Дроид")); // 2800
